...
<head>
    ...
    <title>学生管理 - 批量添加</title>
    <script>
        $(document).ready(function () {
            $('#nav-item-list_students').addClass('active');
        });
    </script>
</head>
<body>
...
<div class="container">
    ...
    try {
        ...
        for ($row_id = 1; $row_id <= 5; ++$row_id) {
            for ($col_id = 1; ; ++$col_id) {
                ...
                if ($uid_col_id === 0) {
                    if ($cell_value === 'xh' || $cell_value === '工号' || $cell_value === '学号' || $cell_value === '学工号') {
                        $uid_col_id = $col_id;
                        continue;
                    }
                }
                if ($name_col_id === 0) {
                    if ($cell_value === 'xm' || $cell_value === '姓名') {
                        $name_col_id = $col_id;
                        continue;
                    }
                }
        ...
        //邮箱可选
        if ($name_col_id === 0 || $uid_col_id === 0 /*|| $email_col_id === 0*/) {
            throw new \Exception('没有找到名为学号、姓名和邮箱的表头。');
        }
        ...
        <div class="row">
            <table class="table table-striped">
                ...
                foreach ($names as $target_uid => $target_name) {
                    ...
                    $exists = (new User($target_uid))->is_existed();
    ...
    <br/>
        ...
        <script>
            ...
            function add_teacher_later(uid, email, name, delay = 0) {
                setTimeout(function () {
                    $.post('/api/account/student/add-student.php', JSON.stringify({
                        'uid': uid,
                        'email': email,
                        'name': name
                    }), function (result) {
    ...
    <?php } else { ?>
        ...
        <div class="row">
            <?php Template::write_alert_div('info', '确保上传的表格含有名为“学号”“姓名”“邮箱”的表头。“邮箱”可留空，但表头必须存在。'); ?>
        </div>
        <div class="row">
